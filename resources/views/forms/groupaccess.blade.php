    ...
    <section class="scrollable padder">
		...
		<div class="m-b-sm">
		    <div class="btn-group btn-group-justified">
		        <a href="{{ URL('/forms/admin/access',$formData[0]['objectId']) }}" class="btn btn-default"><font size="4">USERS</font></a>
		    	<a href="#" class="btn btn-primary disabled"><font size="4">GROUPS</font></a>
		    </div>
		</div>
<div class="tab-content">
	{!! Form::open(array('URL' => '/forms/admin/group/access/{formId}','method'=>'POST')) !!}
	<div class="row">
	    <div class="col-lg-12 margin-tb">
	        <div class="pull-left">
	            <h2><i class="fa fa-group icon"></i>
	            รายชื่อกลุ่มที่มีสิทธิ์เข้าถึงฟอร์ม : {{ $formData[0]['title'] }}</h2>
	        </div>
...
<!--table user access-->
	<table class="table table-responsive no-bordered m-b-none">
		...
		<tr>
			<th width="40%">
				<h4>
				{!! Form::open(['method'=>'GET','url'=>'/forms/admin/group/access','role'=>'search']) !!}
	...
	@foreach ($groups as $key => $group)
		@if(!empty($group->form_ids[0]['form_id']))
			@foreach($group->form_ids as $form)
	...
	<tr>
		...
		@foreach($questionRoleCode as $key => $questionRole)
		<td width="100px">
		{!! Form::open(['method' => 'POST','action' => array('FormsController@groupUpdateAccessQuestion', $formId,$group->id)]) !!}
		...
		@foreach($answerRoleCode as $key => $answerRole)
		<td width="100px">
		{!! Form::open(['method' => 'POST','action' => array('FormsController@groupUpdateAccessAnswer', $formId,$group->id)]) !!}
		...
		@if(Auth::user()->_id == '58fd6cc13fd89d8b529e4acf')
			<td width="80px">
		        {!! Form::open(['method' => 'DELETE','action' => array('FormsController@groupDestroyPermission', $formId,$group->id),'onsubmit' => 'return ConfirmDelete()']) !!}
				{!! Form::button('<i class="fa fa-times"></i>', ['type' => 'submit','class' => 'btn btn-default no-border']) !!}
		        {!! Form::close() !!}
			</td>
		@else
			<td width="80px">
				@if($formData[0]['createdBy'] != $group->_id)
		        	{!! Form::open(['method' => 'DELETE','action' => array('FormsController@groupDestroyPermission', $formId,$group->id),'onsubmit' => 'return ConfirmDelete()']) !!}
					{!! Form::button('<i class="fa fa-times"></i>', ['type' => 'submit','class' => 'btn btn-default no-border']) !!}
		        	{!! Form::close() !!}
	        	@endif
